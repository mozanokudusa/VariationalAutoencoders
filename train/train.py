...
def vae_loss(recon_x, x, mu, logvar, beta=1.0):
    ...
    Returns the total loss, and individual components for logging.
    """
    ...
    # Total loss weighted by beta (annealing factor)
...
def main(config_path):
    ...
    # Annealing Setup
    # Pull kl_warmup_epochs from YAML; default to 0 (no warmup)
    ...
    for epoch in epoch_bar:
        ...
        if warmup_epochs > 0:
            # Linear increase from 0 to final_beta
            current_beta = min(final_beta, (epoch / warmup_epochs) * final_beta)
        else:
        ...
        for batch_x, _ in train_loader:
            ...
            # Unpack all components from vae_loss
        ...
        # Display beta in the progress bar to monitor annealing
        status_msg = f"Epoch {epoch} | Loss: {avg_loss:.2f} | BCE: {bce_averager():.2f} | KLD: {kld_averager():.2f} | Beta: {current_beta:.3f}"
        refresh_bar(epoch_bar, status_msg)
